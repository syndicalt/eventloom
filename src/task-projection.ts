...
/**
 * Rebuilds deterministic software-work task state and projection diagnostics from an ordered Eventloom event history.
 */
...
export function applyTaskEvent(projection: TaskProjection, event: EventEnvelope): TaskProjection {
  ...
  if (!existing) {
    return appendProjectionError(projection, event, "missing_dependency", `Task ${payload.taskId} does not exist`);
  }
...
function applyTaskProposed(projection: TaskProjection, event: EventEnvelope): TaskProjection {
  ...
  if (projection.tasks[payload.taskId]) {
    return appendProjectionError(projection, event, "duplicate_entity", `Task ${payload.taskId} already exists`);
  }

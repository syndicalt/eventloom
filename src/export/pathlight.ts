...
/**
 * Options for exporting Eventloom events to a Pathlight collector.
 *
 * `integrityReport` lets callers export a verified prefix while preserving the
 * original source-log diagnostics in trace metadata and visualizer output.
 */
...
/**
 * Result returned after creating a Pathlight trace.
 *
 * `eventCount` is the number of Pathlight span events written. `exportedEventCount`
 * is the number of Eventloom events exported, and `validPrefixCount` is the
 * replay-safe source prefix represented by the supplied integrity report.
 */
...
export type PathlightExportErrorCode =
  | "pathlight_invalid_base_url"
  | "pathlight_request_failed"
  | "pathlight_invalid_response";

/**
 * Error thrown when the Pathlight collector request or response is not usable.
 */
export class PathlightExportError extends Error {
  readonly suggestedAction = "Check the Pathlight collector URL and retry after the collector accepts requests.";

  constructor(
    readonly code: PathlightExportErrorCode,
    message: string,
    readonly url: string,
    readonly status?: number,
  ) {
    super(message);
    this.name = "PathlightExportError";
  }
}
...
/**
 * Export an Eventloom event sequence to Pathlight without mutating the source log.
 *
 * The function is pure with respect to storage; callers that read from disk should
 * pass `integrityReport` when exporting a verified prefix from a damaged log.
 */
export async function exportToPathlight(
  ...
  const baseUrl = normalizePathlightBaseUrl(options.baseUrl);
  ...
  const baseVisualizer = buildVisualizerModel(events);
  const visualizer = {
    ...baseVisualizer,
    replay: { ...baseVisualizer.replay, integrity },
    handoff: { ...baseVisualizer.handoff, integrity },
  };
  ...
  if (!trace.id) throw new PathlightExportError(
    "pathlight_invalid_response",
    "Pathlight trace create response did not include id",
    `${baseUrl}/v1/traces`,
  );
  ...
  for (const started of actorStartedEvents) {
    ...
    if (!span.id) throw new PathlightExportError(
      "pathlight_invalid_response",
      "Pathlight span create response did not include id",
      `${baseUrl}/v1/spans`,
    );
    spanCount += 1;
...
function normalizePathlightBaseUrl(value: string): string {
  let parsed: URL;
  try {
    parsed = new URL(value);
  } catch {
    throw new PathlightExportError(
      "pathlight_invalid_base_url",
      "Pathlight baseUrl must be an absolute HTTP(S) URL",
      value,
    );
  }
...
async function request(fetcher: typeof fetch, url: string, init: RequestInit): Promise<Response> {
  try {
    ...
    throw new PathlightExportError(
      "pathlight_request_failed",
      `Pathlight request failed: ${url}: ${error instanceof Error ? error.message : String(error)}`,
      url,
    );
  }
}
...
async function parseResponse<T>(response: Response, url: string): Promise<T> {
  if (!response.ok) {
    throw new PathlightExportError(
      "pathlight_request_failed",
      `Pathlight request failed: ${response.status} ${url}`,
      url,
      response.status,
    );
  }

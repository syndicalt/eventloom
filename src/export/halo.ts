...
/**
 * Options for projecting Eventloom events into HALO-compatible OpenTelemetry JSONL.
 *
 * `integrityReport` lets callers export a verified prefix while carrying the
 * source-log diagnostics into the returned result and root span attributes.
 */
...
/**
 * HALO export result with the generated spans and source integrity metadata.
 *
 * `exportedEventCount` is the number of Eventloom events represented in spans.
 * `validPrefixCount` comes from the source verification report when provided.
 */
...
export type HaloExportErrorCode = "halo_provenance_failed";

/**
 * Error thrown when HALO export provenance cannot be collected or supplied.
 */
export class HaloExportError extends Error {
  readonly suggestedAction = "Provide explicit provenance or run the export from a package with readable metadata.";

  constructor(readonly code: HaloExportErrorCode, message: string, cause?: unknown) {
    super(message);
    this.name = "HaloExportError";
    this.cause = cause;
  }
}
...
/**
 * Project an Eventloom event sequence into HALO-compatible span records.
 *
 * Pass `integrityReport` when the input events are a verified prefix from a
 * damaged log so consumers can see the original diagnostics.
 */
export async function exportToHalo(
  events: readonly EventEnvelope[],
  options: HaloExportOptions = {},
): Promise<HaloExportResult> {
  const provenance = options.provenance ?? await collectHaloProvenance(options.provenanceImpl ?? collectRuntimeProvenance);
  ...
  const integrity = options.integrityReport ?? verifyEventChain(events);
  ...
  const root = makeSpan({
    ...
    attributes: {
      ...
      "eventloom.integrity.error_count": integrity.errors.length,
      "eventloom.valid_prefix_count": "validPrefixCount" in integrity ? integrity.validPrefixCount : events.length,
      "eventloom.integrity.diagnostics": JSON.stringify("diagnostics" in integrity ? integrity.diagnostics : integrity.errors),
  ...
  return {
    ...
    projectId,
    traceId,
    traceCount: spans.length > 0 ? 1 : 0,
    spanCount: spans.length,
    exportedEventCount: events.length,
    validPrefixCount: "validPrefixCount" in integrity ? integrity.validPrefixCount : events.length,
    integrity,
    spans,
  };
}

async function collectHaloProvenance(provenanceImpl: () => Promise<RuntimeProvenance>): Promise<RuntimeProvenance> {
  try {
    return await provenanceImpl();
  } catch (error) {
    throw new HaloExportError(
      "halo_provenance_failed",
      `HALO export could not collect runtime provenance: ${error instanceof Error ? error.message : String(error)}`,
      error,
    );
  }
}
...
/**
 * Serialize a HALO export result as newline-delimited JSON span records.
 */

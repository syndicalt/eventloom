...
export interface ResearchProjectionError {
  code: "invalid_payload" | "duplicate_entity" | "missing_dependency" | "invalid_transition";
  eventId: string;
  type: string;
  message: string;
}
...
/**
 * Rebuilds deterministic research-pipeline question, source, claim, challenge, section, and report state from ordered events.
 */
...
function applyQuestionCreated(projection: ResearchProjection, event: EventEnvelope): ResearchProjection {
  ...
  if (projection.questions[payload.questionId]) {
    return appendError(projection, event, "duplicate_entity", `Research question ${payload.questionId} already exists`);
  }
...
function applySourceFound(projection: ResearchProjection, event: EventEnvelope): ResearchProjection {
  ...
  if (question.status !== "created") {
    return appendError(projection, event, "invalid_transition", `Cannot apply ${event.type} to research question ${question.id} in ${question.status} state`);
  }
  if (question.sources.some((source) => source.id === payload.sourceId)) {
    return appendError(projection, event, "duplicate_entity", `Source ${payload.sourceId} already exists`);
  }
...
function applyClaimExtracted(projection: ResearchProjection, event: EventEnvelope): ResearchProjection {
  ...
  if (!question.sources.some((source) => source.id === payload.sourceId)) {
    return appendError(projection, event, "missing_dependency", `Source ${payload.sourceId} does not exist`);
  }
  if (question.claims.some((claim) => claim.id === payload.claimId)) {
    return appendError(projection, event, "duplicate_entity", `Claim ${payload.claimId} already exists`);
  }
...
function applyClaimChallenged(projection: ResearchProjection, event: EventEnvelope): ResearchProjection {
  ...
  if (!question.claims.some((claim) => claim.id === payload.claimId)) {
    return appendError(projection, event, "missing_dependency", `Claim ${payload.claimId} does not exist`);
  }
...
function appendError(
  projection: ResearchProjection,
  event: EventEnvelope,
  code: ResearchProjectionError["code"],
  message: string,
): ResearchProjection {
  return {
    ...projection,

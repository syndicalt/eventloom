...
export type HandoffIntegrityReport = IntegrityReport | EventLogVerificationReport;

export interface HandoffSummary {
  eventCount: number;
  eventTypes: Record<string, number>;
  integrity: HandoffIntegrityReport;
  goals: HandoffGoal[];
  tasks: {
    active: HandoffTask[];
...
/**
 * Summarizes an Eventloom log into goals, task state, decisions,
 * verification, telemetry, risks, and next-action handoff data.
 */
export function summarizeHandoff(
  events: readonly EventEnvelope[],
  integrity: HandoffIntegrityReport = verifyEventChain(events),
): HandoffSummary {
...
/**
 * Formats a handoff summary as stable human-readable Markdown-style text for
 * local review and agent handoff artifacts.
 */

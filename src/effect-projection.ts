...
/**
 * Rebuilds deterministic human-ops effect and approval state, including projection diagnostics, from ordered events.
 */
...
function applyEffectRequested(projection: EffectProjection, event: EventEnvelope): EffectProjection {
  ...
  if (projection.effects[payload.effectId]) {
    return appendError(projection, event, "duplicate_entity", `Effect ${payload.effectId} already exists`);
  }
  ...
  if (payload.target !== undefined) next.target = payload.target;
...
function applyApprovalRequested(projection: EffectProjection, event: EventEnvelope): EffectProjection {
  ...
  if (effect.status !== "requested") {
    return appendError(projection, event, "invalid_transition", `Cannot apply ${event.type} to effect ${effect.id} in ${effect.status} state`);
  }
...
function applyApprovalDecision(
  ...
  if (effect.approvalId !== payload.approvalId) {
    return appendError(projection, event, "invalid_transition", `Approval ${payload.approvalId} does not match effect ${effect.id}`);
  }
...
function appendError(
  projection: EffectProjection,
  event: EventEnvelope,
  code: EffectProjectionError["code"],
  message: string,
): EffectProjection {
  return {
    ...projection,

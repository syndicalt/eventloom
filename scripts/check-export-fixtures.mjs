...
const actualDir = join(root, "fixtures", "export");
const generatedDir = await mkdtemp(join(tmpdir(), "eventloom-export-fixtures-"));
const argv = process.argv.slice(2);
const wantsJson = argv.includes("--json");

try {
  const args = parseFixtureCheckArgs(argv);
  execFileSync("npx", ["tsx", "scripts/generate-export-fixtures.ts", "--out-dir", generatedDir], {
    cwd: root,
  ...
  if (!report.ok) {
    if (!args.json) {
      console.error("Export fixtures are stale. Run npm run fixtures:export and commit the updated fixtures.");

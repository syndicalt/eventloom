...
The envelope is strict. Unknown top-level fields such as `schemaVersion` or
`metadata` are invalid and are reported as `invalid_event_envelope`
diagnostics. Domain evolution belongs inside `payload`, either as optional
payload fields, `payload.version`, or a domain-specific version field.
...
For application-specific workflows, prefer custom intention definitions from the package API instead of editing Eventloom internals. See [Custom Workflows](custom-workflows.md).

When adding a new built-in event type to Eventloom itself:

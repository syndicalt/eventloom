...
Projection errors are useful during replay. Orchestrator validation should prevent invalid accepted events from being appended. Projection-sensitive accepted events must validate and append against the same locked verified snapshot; use the event-store compare-and-append primitive instead of `readAll()` followed by a later `append()`.

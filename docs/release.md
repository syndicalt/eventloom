...
- `.github/workflows/ci.yml` sets `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` and uses Node 24-native GitHub JavaScript actions (`actions/checkout@v5`, `actions/setup-node@v5`, and `actions/upload-artifact@v6`) so release evidence upload does not depend on the deprecated Node 20 action runtime

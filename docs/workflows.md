...
`--resume` keeps the existing log and continues from it. `--max-iterations <n>` bounds a built-in actor loop and is useful for tests, smoke runs, and intentionally partial local workflows.
...
Application-specific workflows do not need to edit Eventloom internals. Use `ActorRegistry`, `Orchestrator` custom intention definitions, and pure projections from package code. See [Custom Workflows](custom-workflows.md) and the runnable `examples/custom-workflow.ts`.

## Adding a Built-In Workflow

Use the existing workflows as templates only when contributing a new built-in workflow to Eventloom itself:

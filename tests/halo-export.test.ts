...
import { exportToHalo, formatHaloJsonl, HaloExportError } from "../src/export/halo.js";
...
describe("HALO export", () => {
  ...
  it("throws typed diagnostics when provenance collection fails", async () => {
    await expect(exportToHalo(taskJournalEvents().slice(0, 1), {
      provenanceImpl: async () => {
        throw new Error("package metadata unavailable");
      },
    })).rejects.toMatchObject({
      name: "HaloExportError",
    ...
    })).rejects.toBeInstanceOf(HaloExportError);
  });
});

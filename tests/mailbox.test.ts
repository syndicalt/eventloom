...
import { ActorRegistry, ActorRegistryError, createSoftwareWorkRegistry } from "../src/actors.js";
...
describe("mailboxes", () => {
  ...
  it("errors clearly for unknown actors", () => {
    ...
    expect(() => buildMailbox(registry, "missing", [])).toThrow(ActorRegistryError);
    try {
      buildMailbox(registry, "missing", []);
      throw new Error("expected missing actor to fail");
    } catch (error) {
      expect(error).toMatchObject({
        name: "ActorRegistryError",
  ...
  it("throws a typed registry error for duplicate actor ids", () => {
    ...
    try {
      ...
      throw new Error("expected duplicate actor to fail");
    } catch (error) {
      expect(error).toMatchObject({
        name: "ActorRegistryError",
        code: "actor_duplicate",
        actorId: "planner",
      });
    }
  });

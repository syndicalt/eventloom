...
import { buildVisualizerModel, renderVisualizerHtml } from "../src/visualizer.js";
...
describe("visualizer model", () => {
  it("builds capture, replay, and handoff views from one event log", () => {
    ...
    expect(model.version).toBe("eventloom.visualizer.v1");
  ...
  it("renders a self-contained static HTML visualizer with escaped embedded data", () => {
    const model = buildVisualizerModel([
      sealEvent(createEvent(event("evt_escape", "goal.created", "user", {
        title: "Preserve </script><script>alert(1)</script> safely",
      })), null),
    ]);

    const html = renderVisualizerHtml(model, { title: "Eventloom Test Log" });

    expect(html).toContain("<!doctype html>");
    expect(html).toContain("<title>Eventloom Test Log</title>");
    expect(html).toContain("Capture");
    expect(html).toContain("Replay");
    expect(html).toContain("Handoff");
    expect(html).toContain(model.replay.projectionHash);
    expect(html).toContain("eventloom-visualizer-data");
    expect(html).not.toContain("</script><script>alert(1)</script>");
    expect(html).toContain("\\u003c/script\\u003e");

    const data = html.match(/<script id="eventloom-visualizer-data" type="application\/json">([\s\S]*?)<\/script>/)?.[1];
    expect(data).toBeTruthy();
    expect(JSON.parse(data ?? "")).toMatchObject({
      version: "eventloom.visualizer.v1",
      capture: {
        eventCount: 1,
        events: [{ summary: "Preserve </script><script>alert(1)</script> safely" }],
      },
      replay: { integrity: { ok: true } },
    });
  });
});
